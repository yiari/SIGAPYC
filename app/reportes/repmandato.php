<?php
	...
	$pdf->SetAutoPageBreak(true, 20);
		...
		if ($tipo_prop == 1){
			...
			$sql = "SELECT p.*,concat(p.nom_prop,' ',p.ape_prop) as nombre,concat(n.nacionalidad,'',p.cedula_prop) as cedula_prop, concat(t.persona,'',p.rif_prop) as rif_prop  ,i.*,u.* FROM `propietario` p left join inmuebles i on i.id_prop = p.`id_prop` left join unidades_inmueble u on u.id_unid =i.`id_inmu` left join nacionalidad n on n.id  = p.id_nacionalidad left join tipo_rif t on t.id_tipo = p.id_rif where p.id_prop='$id_prop'";
